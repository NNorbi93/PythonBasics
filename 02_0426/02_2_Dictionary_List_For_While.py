...
def dict_test():
    ...
    print(
        f"How much is the window: {dict_a_thor.get('window')}")  # Get value - get methodus - None-t dob vissza, ha nem létezik, biztonságosabb
...
def while_true():  # Infinite loop - pl főmenü funckió: menu választásnál amíg nem valid inputot kapsz addig újrafuttatod
    while True:  # amíg igaz ... - itt nincs feltétel
        ...
        if "95" in str(
                timestamp):  # stringgé kell alakítani hogy iterálható legyen, mert a datetime nem string hanem komplex adattipus
